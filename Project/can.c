...
static void CAN_Mode_Config(void)
{
    ...
    CAN_InitStructure.CAN_Prescaler =6;		   ////BTR-BRP 波特率分频器  定义了时间单元的时间长度 36/(1+6+3)/4=0.8Mbps
...
static void CAN_Filter_Config(void)
{
   CAN_FilterInitTypeDef  CAN_FilterInitStructure;
    ...
    CAN_FilterInitStructure.CAN_FilterIdHigh= (((u32)can_rec_id<<3)&0xFFFF0000)>>16;				//要过滤的ID高位 
    CAN_FilterInitStructure.CAN_FilterIdLow= (((u32)can_rec_id<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF; //要过滤的ID低位 
...
{	  
  ...
  TxMessage.ExtId=0x1555aaad;					 //使用的扩展ID
